(ns blender.main
  "Kotodama WASM entrypoint for the Blender clean-room actor (L4) — Clojure port.
  ...
  facts (`blender.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "blender")
(def tier "L4")
...
(def entity-specs
  [{:entity "Project"   :plural "projects"   :id-prefix "blender_pro"
    :fields [:name :ownerId :teamId]              :required [:name]
    :coerce {}                       :refs {}}
   {:entity "File"      :plural "files"      :id-prefix "blender_fil"
    :fields [:projectId :name :contentRef :version] :required [:name :version]
    :coerce {:version :int}          :refs {:projectId "Project"}}
   {:entity "Frame"     :plural "frames"     :id-prefix "blender_fra"
    :fields [:fileId :name :width :height]        :required [:name :width]
    :coerce {:width :float :height :float} :refs {:fileId "File"}}
   {:entity "Component" :plural "components" :id-prefix "blender_com"
    :fields [:fileId :name :description]          :required [:name :description]
    :coerce {}                       :refs {:fileId "File"}}
   {:entity "Comment"   :plural "comments"   :id-prefix "blender_cmt"
    :fields [:fileId :authorId :body :resolved]   :required [:body :resolved]
    :coerce {:resolved :bool}        :refs {:fileId "File"}}
   {:entity "Export"    :plural "exports"    :id-prefix "blender_exp"
    :fields [:fileId :format :scale :contentRef]  :required [:format :scale]
    :coerce {:scale :float}          :refs {:fileId "File"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"blender.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "blender-compat" :tier tier :entities entities} 200])
...
(defn start! [] :blender-compat/ready)
